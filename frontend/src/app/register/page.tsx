...
import { Plane, Mail, Lock, User, ShieldCheck, Sparkles, Navigation } from 'lucide-react';
...
export default function Register() {
  ...
  const onSubmit = async (data: RegisterForm) => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success('Identity established. Initializing Access Key.');
        router.push('/login');
      } else {
        toast.error(result.message || 'Sequence Failure');
      }
    } catch (err) {
      toast.error('Core Connection Lost');
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        <div className="premium-glass p-10 lg:p-12 rounded-[3.5rem] border-white/5 relative z-10">
          ...
          <div className="text-center mb-10">
            ...
            <h1 className="text-3xl font-black text-white uppercase tracking-tighter mb-2">New Identity</h1>
            <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.3em]">Establish Neural Record</p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <input
                  {...register('name')}
                  placeholder="Commander Explorer"
                  className="input-field pl-16 py-5 rounded-[2rem]"
                />
              </div>
              {errors.name && <p className="text-red-400 text-[10px] font-black uppercase ml-1">{errors.name.message}</p>}
            </div>

            <div className="space-y-2">
              <label className="form-label">Email Communications</label>
              <div className="relative group">
                ...
                <input
                  {...register('email')}
                  type="email"
                  placeholder="nexus@exploration.ai"
                  className="input-field pl-16 py-5 rounded-[2rem]"
                />
              </div>
            ...
            <div className="space-y-2">
              <label className="form-label">Sequence Cipher</label>
              <div className="relative group">
            ...
            >
              {loading ? <div className="w-5 h-5 border-2 border-white/20 border-t-white rounded-full animate-spin" /> : <><Navigation className="w-5 h-5" /> Establish Record</>}
            </button>
          </form>

          <p className="text-center mt-10 text-[10px] font-black text-slate-500 uppercase tracking-widest">
            Existing Explorer? <Link href="/login" className="text-blue-400 hover:underline underline-offset-4 ml-1">Portal Access</Link>
          </p>
        </div>
        ...
        <div className="mt-8 flex items-center justify-center gap-6 opacity-30">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[8px] font-black uppercase tracking-widest">Verify Humanity</span>
          </div>
        </div>
      </motion.div>
    </div>
